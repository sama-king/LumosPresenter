...
import type { SongQueueItem } from './songQueue'

interface SongLiveQueueProps {
  queue: SongQueueItem[]
  liveId: string | null
  onPickItem: (item: SongQueueItem) => void
  onClearAll: () => void
}

/**
 * Right column: the live section queue for the active song. Presentational mirror of
 * the scripture LiveQueue, but its items are song sections (no per-item remove — the
 * queue is the whole song's sections, cleared as a unit). ←/→/Space navigate sections
 * from the page-level keyboard handler; clicking an item sends it live directly.
 */
export default function SongLiveQueue({ queue, liveId, onPickItem, onClearAll }: SongLiveQueueProps) {
  return (
      ...
      <div className="panel-scroll min-h-0 flex-1 overflow-y-auto p-3">
        {queue.length === 0 ? (
          <p className="mt-8 text-center font-mono text-mono-ui italic text-slate-muted">
            Pick a section to go live. Use ← / → or Space to advance.
          </p>
        ) : (
                ...
                <li key={item.id}>
                  ...
                  >
                    <span className="flex items-center gap-2">
                      ...
                      <span className="font-mono text-status-label uppercase text-on-surface">
                        {item.label ?? `Section ${item.sectionPosition + 1}`}
                      </span>
                    </span>
